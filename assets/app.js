...
<div class="flex flex-1 flex-col" id="home-sketch">
  ...
  <div class="sk-region sk-feed w-full">
    ...
    <div class="space-y-3">
      <article class="group mb-0 flex flex-col gap-4 overflow-hidden rounded-xl border border-brand-surface bg-white p-4 shadow-sm transition hover:shadow-md sm:flex-row sm:items-stretch sm:gap-5" data-feed-card="true">
        <a href="#" class="hs-feed-thumb relative order-1 block aspect-video w-full shrink-0 overflow-hidden rounded-lg bg-gradient-to-br from-brand-surface to-brand-bg sm:order-1 sm:aspect-[4/3] sm:w-40 sm:max-w-[10rem]">
          <span class="absolute inset-0 bg-gradient-to-br from-brand-surface to-brand-bg" aria-hidden="true"></span>
        </a>
        <div class="order-2 flex min-w-0 flex-1 flex-col justify-between gap-3">
          <div class="min-w-0">
            <h2 class="text-base font-semibold leading-snug text-brand-text sm:text-lg">
              <a href="#" class="text-inherit transition hover:text-brand-accent">Tiêu đề bài — phong cách blog / hướng dẫn</a>
            </h2>
            <p class="mt-2 line-clamp-2 min-h-[2.75rem] text-sm leading-relaxed text-brand-text/65" data-feed-excerpt="true">
              Đoạn mở đầu sau H2 trong bài (mô phỏng nội dung đọc trước, tối đa hai dòng trên giao diện thật).
            </p>
          </div>
          <div class="mt-auto flex items-center gap-2 text-xs text-brand-text/55">
            <i aria-hidden="true" class="far fa-clock text-[13px] opacity-80"></i>
            <span class="tabular-nums">20/03/2026 09:00</span>
          </div>
        </div>
      </article>
      ...
      <article class="group mb-0 flex flex-col gap-4 overflow-hidden rounded-xl border border-brand-surface bg-white p-4 shadow-sm transition hover:shadow-md sm:flex-row sm:items-stretch sm:gap-5" data-feed-card="true">
        ...
        <div class="order-2 flex min-w-0 flex-1 flex-col justify-between gap-3">
          <div class="min-w-0">
            ...
            <p class="mt-2 line-clamp-2 min-h-[2.75rem] text-sm leading-relaxed text-brand-text/65" data-feed-excerpt="true">
              Mô tả ngắn về cách áp dụng trong thực tế; trên blog thật đoạn này lấy từ đoạn văn đầu sau tiêu đề H2.
            </p>
          </div>
          <div class="mt-auto flex items-center gap-2 text-xs text-brand-text/55">
            <i aria-hidden="true" class="far fa-clock text-[13px] opacity-80"></i>
            <span class="tabular-nums">18/03/2026 14:30</span>
          </div>
        </div>
      </article>
      ...
      <article class="group mb-0 flex flex-col gap-4 overflow-hidden rounded-xl border border-brand-surface bg-white p-4 shadow-sm transition hover:shadow-md sm:flex-row sm:items-stretch sm:gap-5" data-feed-card="true">
        ...
        <div class="order-2 flex min-w-0 flex-1 flex-col justify-between gap-3">
          <div class="min-w-0">
            ...
            <p class="mt-2 line-clamp-2 min-h-[2.75rem] text-sm leading-relaxed text-brand-text/65" data-feed-excerpt="true">
              Tóm tắt nội dung chính và điểm cần chú ý trước khi bắt đầu đọc bài đầy đủ.
            </p>
          </div>
          <div class="mt-auto flex items-center gap-2 text-xs text-brand-text/55">
            <i aria-hidden="true" class="far fa-clock text-[13px] opacity-80"></i>
            <span class="tabular-nums">15/03/2026 08:15</span>
          </div>
        </div>
      </article>
    </div>
...
`}function xe(e){let t=e.querySelector(`[data-reading-content]`),n=e.querySelector(`[data-reading-time]`);if(!t||!n)return;let r=(t.textContent||``).trim();if(!r)return;let i=r.split(/\s+/).filter(Boolean).length;n.textContent=`${Math.max(1,Math.ceil(i/200))} phút đọc`}function Se(e){if(e){if(new URLSearchParams(window.location.search).get(`view`)===`post-detail-scratch`){e.innerHTML=be(),l(e),xe(e);return}e.innerHTML=ve(),l(e),ye(e)}}Se(document.querySelector(`#app`));